...
__sbit __at 0xB5 lcdrs ;	//液晶引脚定义
__sbit __at 0xB6 led_cs;
__sbit __at 0xB4 lcden ;
__sbit __at 0xA7 dx;//数码管引脚定义
__sbit __at 0xA6 wx;
...
void write_year(uchar add,uint date)//写时分秒函数
{
    ...
    if (date > 999){
        ...
        write_data(0x30+qian);//送去液晶显示千位
        write_data(0x30+bai);//送去液晶显示百位
...
void keyscan()//按键扫描函数
{
    ...
    if(key1==0) 
    {
        ...
        { 	
            ...
            if(key1num==4)//第三次按下光标闪烁定位到小时位置
            {
                write_com(0x80+12);
            }
            if(key1num==5)//第三次按下光标闪烁定位到小时位置
            {
                write_com(0x80+9);
            }
            if(key1num==6)//第三次按下光标闪烁定位到小时位置
            {
                write_com(0x80+6);
            }
            if(key1num==7)//第四次按下
    ...
    if(key1num!=0)//只有功能键被按下后，增加和减小键才有效
    {
        if(key2==0)
        {
            ...
            if(key2==0)//增加键确认被按下
            {
                ...
                if(key1num==4)//若功能键第三次按下
                {
                    day++;//则调整小时加1
                    if(day==31)//若满30后将清零
                        day=1;
                    write_year(11,day);;//每调节一次送液晶显示一下
                    write_com(0x80+12);//显示位置重新回到调节处
                }
                if(key1num==5)//若功能键第三次按下
                {
                    month++;//则调整小时加1
                    if(month==13)//若满24后将清零
                        month=1;
                    write_year(8,month);;//每调节一次送液晶显示一下
                    write_com(0x80+9);//显示位置重新回到调节处
                }
                if(key1num==6)//若功能键第三次按下
                {
                    year++;//则调整小时加1
                    write_year(3,year);;//每调节一次送液晶显示一下
...
/*中断函数*/
void timer0() __interrupt 1//定时器0中断服务程序
{
                ...
                if(hour==24)//小时加到24则小时清零
                {
                    ...
                    if(day == 31){
                        ...
                        if(month==13){
                            month = 1;
                            year++;
                            write_year(3,year);//年若变化则重新写入
                        }
                        write_year(8,month);//月若变化则重新写入
                    }
                    write_year(11,day);//日若变化则重新写入
                }
